...
// Same structure as Personal details (step-1): header = back, then progress row, then headline; same padding/gap
const FIGMA = {
...
export default function EmailVerificationStep() {
  const { state, updatePersonalData, nextStep, previousStep } = useSignupContext();
  const { showError } = useToast();
  const shakeAnim = useRef(new Animated.Value(0)).current;
  const fadeAnim = useRef(new Animated.Value(0)).current;
  const slideAnim = useRef(new Animated.Value(30)).current;
  const [code, setCode] = useState(() => {
    const s = (state.personalData.verificationCode ?? '').split('').slice(0, DIGIT_COUNT);
  ...
  const email = state.personalData.email || 'your email address';

  useEffect(() => {
    updatePersonalData({ verificationCode: codeString });
  }, [codeString]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      nextStep();
      router.push('/(public)/signup-personal/step-3');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (state.personalData.email && countdown === 0) {
      const t = 60;
      setCountdown(t);
      const timer = setInterval(() => {
        setCountdown((prev) => (prev <= 1 ? (clearInterval(timer), 0) : prev - 1));
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboardView}>
          ...
          >
            <View style={styles.mainContent}>
              ...
              {/* Form + buttons – same as Welcome Back formSection */}
            ...
            {/* Bottom: Didn't receive the code? / Resend code – same structure as previous sign up section */}
