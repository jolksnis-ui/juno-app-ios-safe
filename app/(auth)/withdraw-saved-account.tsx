...
import { getClientFee, SavedBeneficiary } from '../../src/services/withdrawalService';
import { authenticateForCryptoTransaction } from '../../src/services/biometricService';
import { useToast } from '../../src/contexts/ToastContext';

interface SavedAccountFormData {
  selectedCurrency: {
    code: string;
    name: string;
    symbol: string;
...
export default function WithdrawSavedAccountScreen() {
  const params = useLocalSearchParams();
  const styles = useStyles((theme: Theme) => createStyles(theme));
  const { showError } = useToast();
  
  // Parse beneficiary data from params
  const beneficiaryData: SavedBeneficiary = JSON.parse(params.beneficiaryData as string);
  ...
  // Form data
  const [formData, setFormData] = useState<SavedAccountFormData>({
    selectedCurrency: WITHDRAWAL_CURRENCIES.find(c => c.code === beneficiaryData.currency) || WITHDRAWAL_CURRENCIES[0],
    amount: '',
    accountNickname: beneficiaryData.accountNickName,
  ...
  const handleInputChange = (field: keyof SavedAccountFormData, value: string) => {
      ...
      if (authResult.success) {
        ...
        const completeFormData = {
          ...
          beneficiaryCountry: { 
            _id: '', 
            code: '', 
            name: beneficiaryData.beneficiaryCountry, 
            enabled: true, 
            __v: 0 
          },
          beneficiaryAddress: beneficiaryData.beneficiaryAddress,
          bankName: beneficiaryData.bankName,
          bankCountry: { 
            _id: '', 
            code: '', 
            name: beneficiaryData.bankCountry, 
            enabled: true, 
            __v: 0 
          },
          bankAddress: beneficiaryData.bankAddress,
          saveBankAccount: false, // Already saved
          accountNickname: formData.accountNickname,
          accountNumber: '', // Not needed for saved accounts
          sortCode: beneficiaryData.sortCode,
